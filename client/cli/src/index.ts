...
let userToken: string = process.env.USER_TOKEN || "";
...
if (!userToken) {
  if (cmd === "login") {
    const token: string = await logIn(process.argv[3], process.argv[4]);
...
const selectOperations = async () => {
  ...
  if (operation === "quit") {
    ...
    const passwords: Password[] = await allCmd(userToken);
